...
public abstract class Connection {
	 ...
	 * Charges TX energy on the sender (msgFromNode) and notifies the
	 * receiver via messageTransferred().
	 * The message that was being transferred can <STRONG>not</STRONG> be
	 * retrieved from this connection after calling this method (using
	 * {@link #getMessage()}).
	 */
	public void finalizeTransfer() {
		...
		// -------------------------------------------------------------------
		// Charge TX energy on the SENDER (msgFromNode) — this is the only
		// place where we know for certain who sent what and how many bytes.
		// DTNHost.messageTransferred() is called on the RECEIVER, so it
		// cannot reliably charge the sender's battery.
		// -------------------------------------------------------------------
		msgFromNode.getBattery().consumeTx(msgOnFly.getSize());

		// Notify the receiving node that the transfer is complete.
