...
public class DTNSim {
	...
	public static void main(String[] args) {
		...
		// ---------------------------------------------------------------
		// Load group priority → color mappings into NodeGraphic.
		// Must be called AFTER initSettings() so the settings file is
		// fully parsed, and BEFORE the GUI or TextUI starts so every
		// node is drawn in the correct color from the first frame.
		// ---------------------------------------------------------------
		PriorityLoader.load();
		...
		if (batchMode) {
			...
			for (int i=nrofRuns[0]; i<nrofRuns[1]; i++) {
				...
				resetForNextRun();
				// Reload priorities for each batch run in case run-index
				// changes the settings (e.g. different scenario per run).
				PriorityLoader.load();
